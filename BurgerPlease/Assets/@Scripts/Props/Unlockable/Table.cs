...
public class Table : UnlockableBase
{
	...
	private void Start()
	{
		...
		_trashPile.GetComponent<WorkerInteraction>().OnInteraction = OnPlayerTrashInteraction;

		// 돈 인터랙션.
		_moneyPile.GetComponent<WorkerInteraction>().InteractInterval = 0.02f;
		_moneyPile.GetComponent<WorkerInteraction>().OnInteraction = OnPlayerMoneyInteraction;
	}
	...
	#region Interaction
	void OnPlayerTrashInteraction(WorkerController wc)
	{
		// 버거 운반 상태에선 안 됨.
	...
	void OnPlayerMoneyInteraction(WorkerController wc)
	{
